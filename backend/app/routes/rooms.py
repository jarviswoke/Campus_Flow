...
@jwt_required()
def get_rooms():
    """Get all rooms with optional filters — updates live status first"""
    try:
        building  = request.args.get('building')
        status    = request.args.get('status')
        room_type = request.args.get('room_type')

        # ✅ Refresh live occupied/available status from timetable
        ...
        query = Room.query
        if building  and building  != 'all': query = query.filter_by(building=building)
        if status    and status    != 'all': query = query.filter_by(status=status)
        if room_type and room_type != 'all': query = query.filter_by(room_type=room_type)
